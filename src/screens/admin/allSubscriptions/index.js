...
import AddSubscription from '../../../components/overlays/AddSubscriptionOverlay';
import {getAllSubscriptionsType} from '../../../services/subscriptions.services';
import {deleteSubscriptionType} from '../../../services/subscriptions.services';
...
const AllSubscriptions = props => {
  ...
  const handleDelete = async idx => {
    // console.log('subscriptionData =>>>>> ', idx);
    try {
      const response = await deleteSubscriptionType(
        subscriptionData[idx]._id,
        user?.token,
      );
      // console.log(' DELETE deleteSubscriptionType workingggggg', response.data);
      const result = await getAllSubscriptionsType(user?.token);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
            ...
            if (v.type == 'clock only') {
              return (
                <View style={styles.line}>
                  ...
                  <Text style={styles.txt}>${v.cost}</Text>
                  <TouchableOpacity
                    onPress={() => {
                      handleDelete(i);
                    }}>
        ...
        <TouchableOpacity onPress={toggleAdd} style={styles.btn}>
          <Text style={styles.btnTxt}>Add Subscription</Text>
        </TouchableOpacity>

        <AddSubscription
          onBackdropPress={() => toggleAdd(false)}
          isVisible={visible}
        />
      </ScrollView>
    </SafeAreaView>
